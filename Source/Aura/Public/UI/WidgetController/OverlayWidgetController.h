...
struct FUIWidgetRow : public FTableRowBase // inherits from the base class for row structures for DataTables
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly)
	TSubclassOf<class USeeleUserWidget> MessageWidget; // Add a widget so that we could display a little health icon of sorts when pickup
	...
	// Delegate to broadcast DataTableRow with message. Tags and corresponding info (whatever is listen in struct above)
